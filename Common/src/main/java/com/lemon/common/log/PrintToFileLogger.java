...
import com.lemon.common.util.AndroidVersionCheckUtils;
import com.lemon.common.util.ExternalOverFroyoUtils;
import com.lemon.common.util.ExternalUnderFroyoUtils;
...
public class PrintToFileLogger implements ILogger
{
	...
	public void open()
	{
		...
		{
			logDir = ExternalOverFroyoUtils.getDiskCacheDir(mContext.getApplicationContext(), LOG_DIR);
		} else
		{
			logDir = ExternalUnderFroyoUtils.getDiskCacheDir(mContext.getApplicationContext(), LOG_DIR);
		}
		if (!logDir.exists())
		{
			logDir.mkdirs();
